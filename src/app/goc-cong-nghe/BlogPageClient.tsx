...
export default function BlogPageClient({ posts }: Props) {
  ...
  const sidebarFeatured = posts.slice(1, 4);
  const weeklyPopular = posts.slice(0, 6);

  // Filter posts by category for news section
  const newsFiltered = activeTab === 'all' ? posts : posts.filter(p => p.category === activeTab);
  const newsTotal = newsFiltered.length;
  const newsPosts = newsFiltered.slice((newsPage - 1) * perPage, newsPage * perPage);
  const newsTotalPages = Math.ceil(newsTotal / perPage);

  const tipsPosts = posts.filter(p => p.category === 'phu-kien' || p.category === 'ipad').slice(0, 5);
  ...
  if (!featured) return <div className="max-w-7xl mx-auto px-4 py-12 text-center text-text-muted">Chưa có bài viết nào.</div>;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-4">
          <Link href={`/goc-cong-nghe/${featured.slug}`} className="group relative block bg-bg-gray rounded-lg overflow-hidden lg:row-span-3">
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent flex flex-col justify-end p-5">
              ...
              <h3 className="text-lg font-bold text-white group-hover:opacity-90 line-clamp-2">{featured.title}</h3>
          ...
          {sidebarFeatured.map((post) => (
            <Link key={post.id} href={`/goc-cong-nghe/${post.slug}`} className="group flex gap-3 bg-white border border-border rounded-lg p-3 items-center">
              ...
              <div className="min-w-0 flex-1">
                <h4 className="text-sm font-semibold text-text-dark group-hover:text-navy transition-colors line-clamp-2 leading-tight">{post.title}</h4>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
          {weeklyPopular.map((post) => (
            <Link key={post.id} href={`/goc-cong-nghe/${post.slug}`} className="group block bg-white border border-border rounded-lg overflow-hidden">
              ...
              <div className="p-2">
                <h4 className="text-xs font-medium text-text-dark group-hover:text-navy transition-colors line-clamp-2 leading-tight">{post.title}</h4>
                ...
                {tabs.map((tab) => (
                  <button
                    ...
                    className={`px-2.5 py-1 text-xs rounded transition-colors ${
                      activeTab === tab.key ? 'bg-navy text-white' : 'text-text-muted hover:text-text-dark'
                    }`}
                  >
                    {tab.label}
                  </button>
            ...
            <div className="space-y-4">
              {newsPosts.map((post) => (
                <Link key={post.id} href={`/goc-cong-nghe/${post.slug}`} className="group flex gap-4 pb-4 border-b border-border last:border-0">
                  ...
                  <div className="min-w-0 flex-1">
                    <h3 className="text-sm font-bold text-text-dark group-hover:text-navy transition-colors line-clamp-2 leading-snug">{post.title}</h3>
            ...
            <div className="space-y-3">
              {tipsPosts.map((post) => (
                <Link key={post.id} href={`/goc-cong-nghe/${post.slug}`} className="group flex gap-3 items-start">
                  ...
                  <div className="min-w-0 flex-1">
                    <h4 className="text-xs font-semibold text-text-dark group-hover:text-navy transition-colors line-clamp-2 leading-tight">{post.title}</h4>
