...
import { getBlogPostBySlug, getRelatedBlogPosts, blogPosts } from '@/data/blog-posts';
  ...
  if (!post) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-12 text-center">
        ...
        <Link href="/goc-cong-nghe" className="mt-4 inline-block text-navy link-hover">
          Quay lại Góc công nghệ
        </Link>
      </div>
    );
  }
    ...
    <div className="max-w-7xl mx-auto px-4 py-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-8 mt-6">
        ...
        <article>
          ...
          {/* Excerpt */}
          <p className="mt-6 text-base text-text-muted leading-relaxed italic border-l-4 border-navy pl-4">
            {post.excerpt}
          </p>
          ...
          <div className="mt-8 pt-6 border-t border-border flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs bg-bg-gray text-text-muted px-2.5 py-1 rounded-full">
                {blogCategoryLabels[post.category] || post.category}
              </span>
              <span className="text-xs bg-bg-gray text-text-muted px-2.5 py-1 rounded-full">
                Apple
          ...
          <div className="border border-border rounded-xl overflow-hidden">
            ...
            <div className="divide-y divide-border">
              {recentPosts.map((p, i) => (
                <Link
                  key={p.id}
                  href={`/goc-cong-nghe/${p.slug}`}
                  className="flex items-start gap-3 p-3 hover:bg-bg-gray transition-colors"
                >
                  <span className="shrink-0 w-7 h-7 rounded-full bg-navy/10 text-navy text-xs font-bold flex items-center justify-center mt-0.5">
                    {i + 1}
                  </span>
                  <div className="min-w-0">
                    <h4 className="text-sm font-medium text-text-dark line-clamp-2 leading-snug">
                      {p.title}
                    </h4>
          ...
          <div className="border border-border rounded-xl overflow-hidden">
            ...
            <div className="p-3 space-y-1">
              {Object.entries(blogCategoryLabels).map(([key, label]) => (
                <Link
                  key={key}
                  href={`/goc-cong-nghe?tab=${key}`}
                  className="flex items-center justify-between py-2 px-2 text-sm text-text-muted hover:text-navy hover:bg-bg-gray rounded-lg transition-colors"
                >
                  <span>{label}</span>
                  <ChevronRight className="h-3.5 w-3.5" />
                </Link>
