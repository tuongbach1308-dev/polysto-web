          ...
          <div className="lg:col-span-1 space-y-4">
            {specEntries.length > 0 && (
              <div className="bg-white border border-gray-200 rounded-lg p-5 sticky top-[120px] space-y-5">
                ...
                {relatedPosts.length > 0 && (<div className="border-t border-gray-200 pt-5"><h3 className="text-sm font-bold text-gray-900 uppercase tracking-wide mb-3 flex items-center gap-2"><Newspaper size={15} className="text-brand-500" /> Tin tức sản phẩm</h3><div className="space-y-3">{relatedPosts.map((post) => (<Link key={post.id} href={`/tin-tuc/${post.slug}`} className="group flex gap-2.5"><div className="w-[60px] h-[44px] flex-shrink-0 rounded overflow-hidden bg-gray-100">{post.thumbnail ? (<img src={post.thumbnail} alt={post.title} className="w-full h-full object-cover" />) : (<div className="w-full h-full bg-brand-700 flex items-center justify-center"><span className="text-white/30 text-[8px] font-black">POLY</span></div>)}</div><div className="flex-1 min-w-0"><h4 className="text-[12px] text-gray-700 leading-snug line-clamp-2 group-hover:text-brand-600 transition-colors">{post.title}</h4><p className="text-[10px] text-gray-400 mt-0.5">{formatDate(post.created_at)}</p></div></Link>))}</div></div>)}
              </div>
            )}
          </div>
        </div>
